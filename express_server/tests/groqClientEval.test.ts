...
import { GroqClient, ChatResponse, Message } from "../src/groq-client"; // Adjust path to your GroqClient file
...
describe("GroqClient Tool Call Evals", () => {
  ...
  const testCases: [string, Message[], string | null][] = [
    [
    ...
    [
      ...
      [
        { role: "system", content: "You are a helpful assistant." },
        {
          role: "user",
          content:
            "Search for emails from test@example.com in the last 2 days.",
        },
      ],
  ...
  testCases.forEach(([name, messages, expectedTool]) => {
    test(
      name,
      async () => {
        const response: ChatResponse = await client.chat(messages, true);
